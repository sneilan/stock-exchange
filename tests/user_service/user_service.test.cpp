...
TEST_CASE("LibSodium Test - Creating and authenticating password") {
  ...
  REQUIRE(crypto_pwhash_str_verify(hashed_password, my_password,
                                   strlen(my_password)) == 0);

  // Make sure wrong password does not.
  const char *wrong_password = "battery staple";
  REQUIRE(crypto_pwhash_str_verify(hashed_password, wrong_password,
                                   strlen(wrong_password)) != 0);
}
...
TEST_CASE("UserService - Create database, add user and try logging in") {
  ...
  int rc = system(
      "/app/scripts/operations/create_databases.py --user-db /tmp/users.db");
  REQUIRE(rc == 0);

  rc = system("/app/scripts/operations/add_user.py --username sneilan "
              "--password password --user-db /tmp/users.db");
  REQUIRE(rc == 0);
